...
const PROJECT_TRANSLATIONS = {
  en: {
    ...
    'ai-analytics-assistant': {
      ...
      tagline:  'Natural language → SQL → results, no data team bottleneck',
      desc:     'LLM-powered assistant that converts natural language questions into SQL, executes them against a PostgreSQL database, and returns chart-ready results with explanations. Supports both OpenAI and local Ollama models.',
      highlights: [
        'Schema-aware prompting: injects table DDL + sample rows into LLM context for accurate SQL',
        'Dual model support — OpenAI GPT-4 for production, Ollama (llama3) for local/offline use',
        'Query safety layer: read-only executor, SQL injection pattern detection',
        'Result auto-charting: LLM decides chart type (bar, line, table) based on query semantics',
        'Conversation memory: follow-up questions maintain context of previous queries',
  ...
  vi: {
    ...
    'ai-analytics-assistant': {
      ...
      tagline:  'Câu hỏi tự nhiên → SQL → kết quả, không cần qua data team',
      desc:     'Trợ lý dùng LLM chuyển câu hỏi ngôn ngữ tự nhiên thành SQL, thực thi trên PostgreSQL và trả về kết quả kèm biểu đồ và giải thích. Hỗ trợ cả OpenAI và Ollama local.',
      highlights: [
        'Prompt hiểu schema: inject DDL + sample row vào context LLM để sinh SQL chính xác',
        'Hỗ trợ 2 model — GPT-4 cho production, Ollama (llama3) cho dùng offline',
        'Lớp bảo vệ query: chỉ đọc, phát hiện pattern SQL injection',
        'Tự chọn loại chart: LLM quyết định bar/line/table dựa theo ngữ nghĩa query',
        'Bộ nhớ hội thoại: câu hỏi tiếp theo giữ nguyên context từ query trước',
        'Giao diện Streamlit: lịch sử query, copy-to-clipboard, toggle SQL explain',
      ],
    },
    'data-governance': {
